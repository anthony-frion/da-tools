...
def generate_mask(
    ...
    if constant_obs_count_per_step:
        if shape.numel() > 1e9:
            warnings.warn('Attempting to generate a high-dimensional mask, memory issues may occur. If so, try setting constant_obs_count_per_step to False for a less memory-hungry computation, or reducing the number of time steps.')
