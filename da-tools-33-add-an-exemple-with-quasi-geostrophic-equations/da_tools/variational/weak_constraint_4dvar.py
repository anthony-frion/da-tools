...
class WC4DVarLoss(_Loss):
    ...
    def forward(self, fields: TensorDict) -> Tensor:
        ...
        logp = (
            0 if self.background_prior is None else self.background_prior.log_prob(self.x[:, :1]).squeeze(1)
        )
        # logp now has only a single singleton batch dimension

        logp = logp + self.obs_op.log_prob(self.x, self.observations)
...
def wc4dvar_single_window(
    ...
    fields = optimize(loss, x_init.fields, optimizer_class, optimizer_pars, n_steps, verbose)
