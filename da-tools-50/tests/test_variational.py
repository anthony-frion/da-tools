...
class TestVariational(TestCase):
    ...
    def test_hc4dvar_singlewindow_with_normalization(self):
        noise_sd = torch.rand(1).item() * 0.2 + 0.9

        x_all, y_all, obs_op, prior, noise_sd = generate_testdata_hc_id(T=5, obs_noise_sd=noise_sd, use_tensordict=True)
