...
Original code: https://github.com/louity/qgm_pytorch
...
def compute_laplace_dst(nx, ny, dx, dy, arr_kwargs):
    """Discrete sine transform of the 2D centered discrete laplacian operator."""
    x, y = torch.meshgrid(torch.arange(1, nx - 1, **arr_kwargs), torch.arange(1, ny - 1, **arr_kwargs), indexing="ij")
    return (
        2 * (torch.cos(torch.pi / (nx - 1) * x) - 1) / dx**2 + 2 * (torch.cos(torch.pi / (ny - 1) * y) - 1) / dy**2
    )
...
def inverse_elliptic_dst(f, operator_dst):
    ...
    return dstI2D(dstI2D(f.type(torch.float32)) / operator_dst).type(f.dtype)
...
def jacobi_h(f, g):
    ...
    Scalar fields f and g must have the same dimension. Grid is regular and dx = dy.
    """
...
def laplacian_h_nobc(f):
    "Computes the laplacian with no boundary conditions"
    return f[..., 2:, 1:-1] + f[..., :-2, 1:-1] + f[..., 1:-1, 2:] + f[..., 1:-1, :-2] - 4 * f[..., 1:-1, 1:-1]


def matmul(M, f):
    """Matrix multiplication between M and f."""
...
def curl_wind(tau, dx, dy):
    """Computes the wind curl associated to inputs dx and dy."""
...
class QGM:
    """Implementation of multilayer quasi-geostrophic model in variables pressure p and potential vorticity q."""
    ...
    def __init__(self, param):
        ...
        self.zfbc = self.bcco / (1.0 + 0.5 * self.bcco)
        ...
        self.inverse_elliptic_dst = torch.jit.trace(
            inverse_elliptic_dst, (self.q_over_f0[..., 1:-1, 1:-1], self.helmoltz_dst)
        )
    ...
    def compute_A_matrix(self):
        """Computes and stores the A matrix associated to the current state."""
        ...
        for i in range(1, self.nl - 1):
            ...
            A[i, i] = 1.0 / self.heights[i] * (1 / self.reduced_gravities[i] + 1 / self.reduced_gravities[i - 1])
            A[i, i + 1] = -1.0 / (self.heights[i] * self.reduced_gravities[i])
        A[-1, -1] = 1.0 / (self.heights[self.nl - 1] * self.reduced_gravities[self.nl - 2])
        A[-1, -2] = -1.0 / (self.heights[self.nl - 1] * self.reduced_gravities[self.nl - 2])
    ...
    def compute_helmoltz_matrices(self):
        """Computes and stores the Helmholtz matrices associated to the current state."""
        self.helmoltz_dst = compute_laplace_dst(self.nx, self.ny, self.dx, self.dy, self.arr_kwargs).reshape(
            (1, self.nx - 2, self.ny - 2)
        ) / self.f0**2 - self.lambd.reshape((self.nl, 1, 1))
        ...
        self.homogeneous_sol = (constant_field + s_solutions * self.lambd.reshape((self.nl, 1, 1)))[
            :-1
    ...
    def compute_alpha_matrix(self):
        """Computes and stores the alpha matrix associated to the current state."""
        (Cm2l, Cl2m, hom_sol) = (
    ...
    def compute_q_over_f0_from_p(self):
        """Computes and stores the value of q_over_f0 using p."""
        first_nonunit_dim = 0
        while first_nonunit_dim < len(self.p.shape) and self.p.shape[first_nonunit_dim] == 1:
            first_nonunit_dim += 1
        if self.p.shape[first_nonunit_dim] == self.A.shape[-1]:
            Ap = (self.A @ self.p.reshape(self.p.shape[: len(self.p.shape) - 2] + (-1,))).reshape(self.p.shape)
        else:
            #print(self.p.shape)
            A_batch = self.A.unsqueeze(0).expand(self.p.shape[first_nonunit_dim], *self.A.shape)
            p_batch = self.p.reshape(self.p.shape[first_nonunit_dim], self.p.shape[first_nonunit_dim+1], -1)
            #print(A_batch.shape, p_batch.shape)
            Ap = torch.bmm(A_batch, p_batch).reshape(self.p.shape)
        #print(self.p.shape, self.zfbc)
        #print("compute_q_over_f0_from_p")
        #print(self.p.dtype, Ap.dtype)
        self.q_over_f0 = (
            laplacian_h(self.p, self.zfbc) / (self.f0 * self.dx) ** 2 - Ap + (self.beta / self.f0) * (self.y - self.y0)
        ).to(self.p.dtype)
    ...
    def advection_rhs(self):
        ...
        #print("Beginning of advection_rhs:")
        #print(self.q_over_f0.dtype, self.p.dtype)
    ...
    def compute_time_derivatives(self, update_PV=True):
        ...
        if update_PV:
            dp_bound = torch.cat(
                [self.dp[..., 0, 1:-1], self.dp[..., -1, 1:-1], self.dp[..., :, 0], self.dp[..., :, -1]], dim=-1
            )
